...
,max(case when pvl.npi>'' then pvl.npi
               ...
               else case when pid.type_rtk='C3VD0FMN8L' then pid.id end end) as PCLnpi, pcla.address_k
into #ProvGroup
...
group by pcpa.provider_K, p.npi, pcl.Group_K, pcl.ID, coalesce(pvl.longname,pcl.groupname), pvl.provider_k, pcla.addresstype_rtk, pcla.address_k
...
select pcp.NPI, pcp.Provider_K, max(pcp.LONGNAME) as ProvName, min(rt.description) as status,  -- 'Contracted'<'Credentialed' so takes priority
   ...
   ,cpc.PCPlocation, cpc.Addresstype, cpc.LocationType, cpc.CactusID, cpc.PCPlocProvider_K, cpc.ProviderCat, cpc.PCLcat, cpc.address_k
  into #cactusPCP   
   from CactusDBSrv.Cactus.VISUALCACTUS.PROVIDERS as pcp
	 ...
	 left join 
	(SELECT PROVIDER_K, CactusID, PCPlocation, AddressType, LocationType, ProviderCat, PCLcat, PCPlocProvider_K, address_k FROM #ProvGroup p
	 ...
	 group by provider_k, PCPlocProvider_K, pcplocation, cactusid, AddressType, LocationType, ProviderCat, PCLcat, address_k
		) cpc
   ...
   where (id.USERDEF_RTK2 is not null or ea.CATEGORY_RTK in ('C3VD0FMMRO','#FAB7BCCF1','#0A0C781AC') or rp.description='PCP')
	 ...
	 and ea.active=1
   group by pcp.NPI, pcp.Provider_K, cpc.AddressType, cpc.PCPlocation, cpc.CactusID, cpc.PCPlocProvider_K, cpc.LocationType, cpc.ProviderCat, cpc.PCLcat, cpc.address_k
   having pcp.NPI>''
   
create index npi on #cactusPCP (npi) 
-- select * from #cactusPCP where npi in (select npi from #CactusPCP group by npi having count(*)>1) order by npi
...
, a.AddressLine1, a.AddressLine2, a.City, a.County, a.State, a.ZipCode
from #CactusPCP c left join CactusDBSrv.[Cactus].[VISUALCACTUS].[ADDRESSES] a on c.address_k=a.address_k
